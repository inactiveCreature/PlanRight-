...
export function Field({
  ...
  return (
    <div className={`space-y-2 ${className || ''}`}>
      ...
      <div
        className={`transition-all duration-200 ${error ? 'ring-2 ring-red-200' : success ? 'ring-2 ring-green-200' : ''}`}
      >
        {children}
      </div>
      {error && (
...
export function TextInput({
  ...
  const baseClasses =
    'w-full rounded-xl border px-grid-md py-grid-sm text-slate-900 focus-ring transition-all duration-200'
...
export const Tooltip: React.FC<{ children: React.ReactNode; content: string }> = ({
  children,
  content,
}) => {
