...
export default function AssistantPanel({ role, className = '' }: AssistantPanelProps) {
  ...
  return (
    <div className={`min-w-0 flex flex-col rounded-3xl border border-slate-200/60 bg-white shadow-xl backdrop-blur-sm max-h-[calc(100vh-80px-24px)] overflow-hidden ${className}`}>
      {/* Header */}
      ...
      >
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.who === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in duration-200`}>
        ...
        <form onSubmit={onSend} className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            <textarea
              ...
              placeholder={isConnected ? "Ask about rules, set form values, or get help..." : "Ollama not connected - basic commands only"} 
              disabled={isLoading}
              rows={1}
            ...
            {input.trim() && !isTyping && (
              <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none">
                <div className="flex items-center gap-2 bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-lg border border-slate-200/60 shadow-lg animate-in fade-in duration-200">
                  <kbd className="px-2 py-1 text-xs font-semibold text-slate-700 bg-slate-100 border border-slate-300 rounded-lg shadow-sm">Enter</kbd>
