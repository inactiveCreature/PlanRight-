...
export default function DecisionCard({ result, role = 'Resident' }: { result: RuleResult | null, role?: UserRole }) {
  if (!result) return null
  
  const isExempt = result.decision === 'Likely Exempt'
  const isNotExempt = result.decision === 'Likely Not Exempt'
  const cannotAssess = result.decision === 'Cannot assess'
  ...
  return (
    <div className={`border-2 rounded-2xl p-6 shadow-lg transition-all duration-300 max-w-[960px] mx-auto ${
      isExempt 
          ...
          {result.checks.slice(0, 6).map((c) => (
            <li key={c.rule_id} className="flex items-start gap-3">
              ...
              <span className="text-sm">
                ...
                <span className={`ml-1 font-semibold ${c.pass ? 'text-green-800' : 'text-red-800'}`}>
                  {c.pass ? 'pass' : 'fail'}
                </span> 
            ...
            {result.checks.filter(c => !c.pass).map((c) => (
              <li key={c.rule_id} className="flex items-start gap-3">
                ...
                <span className="text-sm text-red-800">
                  {c.rule_id === 'G-AREA-1' ? 'Area calculation mismatch — check length × width' :
                   c.rule_id === 'G-SITING-1' ? 'On easement — relocate off easement' :
                   c.rule_id === 'G-HERITAGE-1' ? 'Heritage/conservation restrictions — seek council advice' :
                   c.rule_id === 'S-BBL-1' || c.rule_id === 'P-BBL-1' || c.rule_id === 'C-BBL-1' ? 'Not behind building line — relocate behind building line' :
                   c.rule_id === 'S-FRONT-1' || c.rule_id === 'P-FRONT-1' || c.rule_id === 'C-FRONT-1' ? 'Insufficient front setback — increase to ≥5.0m' :
                   c.rule_id === 'S-HEIGHT-1' ? 'Height exceeds 3.0m — reduce height' :
                   c.rule_id === 'S-AREA-1' ? 'Area exceeds 20m² — reduce area' :
                   c.rule_id === 'S-SIDE-1' ? 'Insufficient side setback — increase to ≥0.9m' :
                   c.rule_id === 'S-REAR-1' ? 'Insufficient rear setback — increase to ≥0.9m' :
                   c.rule_id === 'S-SEWER-1' ? 'Over sewer — relocate away from sewer' :
                   c.rule_id === 'S-ATTACH-1' ? 'Shed attached to dwelling — detach' :
                   c.rule_id === 'S-FLOOD-1' ? 'Flood prone — seek council advice' :
                   c.rule_id === 'S-BUSHFIRE-1' ? 'Bushfire prone — seek council advice' :
