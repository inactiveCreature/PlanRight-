...
import { Field, TextInput, Select, Chip, Tooltip } from '../../components/ui/FormBits'
...
import { run_rules_assessment, list_sample_properties } from '../../rules/engine'
import { assess } from '../../assessment/assess'
import { usePlanRightStore } from '../../store'
import { getThresholds } from '../../config/thresholds'
import { validateStep, getFriendlyErrorMessage, isFieldError, type StepId } from '../../wizard/steps'
...
export default function PlanRightWizard({ role, onChangeRole }: { role: UserRole, onChangeRole: (r: UserRole)=>void }) {
  ...
  }, [proposal.dimensions.length_m, proposal.dimensions.width_m, manualArea, proposal.dimensions.area_m2, setField])

  // Get current step validation
  const currentStepId = STEPS[current].id as StepId
  const { valid: isCurrentStepValid, errors: currentStepErrors } = validateCurrentStep(currentStepId)
  ...
  const getFieldError = (fieldPath: string): string | undefined => {
    const error = currentStepErrors.find((e: { path: string; message: string }) => e.path === fieldPath)
          ...
          <Card>
            <div className="grid grid-cols-12 gap-4">
              ...
              <div className="col-span-7 md:col-span-12">
                <label className="block text-sm font-medium text-slate-800 mb-1">Assessment type</label>
        ...
        <>
            ...
            {useSample && (
              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-8 md:col-span-12">
                  <label className="block text-sm font-medium text-slate-800 mb-1">Sample property</label>
                  <select value={selectedSampleId} onChange={(e) => setSelectedSampleId((e.target as HTMLSelectElement).value)} className="w-full rounded-xl border border-slate-300 px-3 py-2 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-600">
                    <option value="">— Select —</option>
                    {SAMPLE_PROPERTIES.map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
                  </select>
                </div>
          ...
          <Card className="mb-6">
            <div className="space-y-4">
              ...
              <NumberField
                  label="Lot size (m²)" 
                      value={typeof proposal.property.lot_size_m2 === 'number' ? proposal.property.lot_size_m2 : undefined}
              ...
              <NumberField
                  label="Frontage (m)" 
                      value={typeof proposal.property.frontage_m === 'number' ? proposal.property.frontage_m : undefined}
        ...
        <>
          ...
          <Card>
            <div className="space-y-4">
              <NumberField
                  label="Length (m)" 
                      value={typeof proposal.dimensions.length_m === 'number' ? proposal.dimensions.length_m : undefined}
              ...
              <NumberField
                  label="Width (m)" 
                      value={typeof proposal.dimensions.width_m === 'number' ? proposal.dimensions.width_m : undefined}
              ...
              <NumberField
                  label="Height (m)" 
                      value={typeof proposal.dimensions.height_m === 'number' ? proposal.dimensions.height_m : undefined}
              ...
              />
                        <NumberField 
                label="Area (m²)"
                          value={typeof proposal.dimensions.area_m2 === 'number' ? proposal.dimensions.area_m2 : undefined}
                          onCommit={(v) => { setManualArea(true); setField('dimensions.area_m2', v) }} 
                          min={0}
                          step={0.1}
                          placeholder="7.2"
                          suffix="m²"
            ...
            <div className="space-y-4">
              ...
              <NumberField
                  label="Front setback (m)" 
                      value={typeof proposal.location.setback_front_m === 'number' ? proposal.location.setback_front_m : undefined}
              ...
              <NumberField
                  label="Side setback (m)" 
                      value={typeof proposal.location.setback_side_m === 'number' ? proposal.location.setback_side_m : undefined}
              ...
              <NumberField
                  label="Rear setback (m)" 
                      value={typeof proposal.location.setback_rear_m === 'number' ? proposal.location.setback_rear_m : undefined}
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-800">
                    {proposal.location.behind_building_line_bool ? (
                      <>✅ <strong>Behind building line:</strong> Front setback is not required. Your structure is positioned behind the building line (typically 5m from street frontage).</>
                    ) : (
                      <>⚠️ <strong>In front of building line:</strong> Front setback is required and must meet the minimum distance (5.0m) from the street frontage.</>
                    )}
                  </p>
                </div>
              </div>
    ...
    if (stepId === 'review') {
      return (
        <>
          <PageHeader 
            title="Review" 
            subtitle="Confirm entries then run the rules check."
          >
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="bg-white border border-slate-200 rounded-xl p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div><span className="text-slate-500">Type:</span> <span className="font-medium">{proposal.structure.type || '—'}</span></div>
                    <div><span className="text-slate-500">Dimensions:</span> <span className="font-medium">L {proposal.dimensions.length_m || '—'}m × W {proposal.dimensions.width_m || '—'}m × H {proposal.dimensions.height_m || '—'}m</span></div>
                    <div><span className="text-slate-500">Area:</span> <span className="font-medium">{proposal.dimensions.area_m2 || '—'} m²</span></div>
                  </div>
                </div>
                <div className="bg-white border border-slate-200 rounded-xl p-4">
                  <h5 className="font-semibold text-slate-800 mb-3">Setbacks</h5>
                  <div className="space-y-2 text-sm">
                    <div><span className="text-slate-500">Front:</span> <span className="font-medium">{proposal.location.setback_front_m || '—'} m</span></div>
                    <div><span className="text-slate-500">Side:</span> <span className="font-medium">{proposal.location.setback_side_m || '—'} m</span></div>
                    <div><span className="text-slate-500">Rear:</span> <span className="font-medium">{proposal.location.setback_rear_m || '—'} m</span></div>
                  </div>
                </div>
                <div className="bg-white border border-slate-200 rounded-xl p-4">
                  <h5 className="font-semibold text-slate-800 mb-3">Location & Siting</h5>
                  <div className="space-y-2 text-sm">
                    <div><span className="text-slate-500">Behind building line:</span> <span className={`font-medium px-2 py-1 rounded-full text-xs ${proposal.location.behind_building_line_bool ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{String(proposal.location.behind_building_line_bool)}</span></div>
                    <div><span className="text-slate-500">On easement:</span> <span className={`font-medium px-2 py-1 rounded-full text-xs ${proposal.siting.on_easement_bool ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>{String(proposal.siting.on_easement_bool)}</span></div>
                    <div><span className="text-slate-500">Over sewer:</span> <span className={`font-medium px-2 py-1 rounded-full text-xs ${proposal.siting.over_sewer_bool ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>{String(proposal.siting.over_sewer_bool)}</span></div>
                  </div>
                </div>
                <div className="bg-white border border-slate-200 rounded-xl p-4">
                  <h5 className="font-semibold text-slate-800 mb-3">Context Flags</h5>
                  <div className="space-y-2 text-sm">
                    <div><span className="text-slate-500">Heritage:</span> <span className={`font-medium px-2 py-1 rounded-full text-xs ${proposal.context.heritage_item_bool ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-800'}`}>{String(proposal.context.heritage_item_bool)}</span></div>
                    <div><span className="text-slate-500">Conservation:</span> <span className={`font-medium px-2 py-1 rounded-full text-xs ${proposal.context.conservation_area_bool ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-800'}`}>{String(proposal.context.conservation_area_bool)}</span></div>
                    <div><span className="text-slate-500">Flood prone:</span> <span className={`font-medium px-2 py-1 rounded-full text-xs ${proposal.context.flood_prone_bool ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>{String(proposal.context.flood_prone_bool)}</span></div>
                    <div><span className="text-slate-500">Bushfire prone:</span> <span className={`font-medium px-2 py-1 rounded-full text-xs ${proposal.context.bushfire_bool ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>{String(proposal.context.bushfire_bool)}</span></div>
                  </div>
                </div>
              </div>
            </Card>
  ...
  function updateFormFields(values: Partial<Record<string, any>>) {
    ...
    Object.keys(values).forEach((k) => {
      if (
        ['propertyAddress', 'residentZone', 'heritageArea', 'conservationArea', 'floodProne', 'bushfireProne', 'planningRaw']
          .includes(k)
      ) return;
      // If caller passed keys like "property.id" or "context.some_flag", forward them
      if (k.includes('.')) {
        // @ts-ignore - setField accepts string path
            ...
            <div ref={errorSummaryRef} className="mb-6">
              <Card className="border-warning bg-warning">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-warning mb-2">Please complete the required fields</h3>
                    <ul className="text-sm text-warning space-y-1">
                      {currentStepErrors.map((error: { path: string; message: string }, index: number) => (
                        <li key={index}>• {getFriendlyErrorMessage(error.path, error.message)}</li>
                      ))}
                    </ul>
                  </div>
                </div>
