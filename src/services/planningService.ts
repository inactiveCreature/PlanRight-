...
async function fetchLayerIntersect(
  type: 'property' | 'point' = 'property',
  id: string,
  layers = 'epi'
) {
...
function parseLayerIntersectPayload(payload: any) {
  ...
  for (const layer of layers) {
    ...
    const results = Array.isArray(layer.results)
      ? layer.results
    ...
    if (!out.heritageArea) {
      if (
        ...
        results.some((r: any) =>
          Object.keys(r).some(
            (k) => k.toLowerCase().includes('heritage') || k.toLowerCase().includes('heritage type')
          )
        )
      ) {
    ...
    if (!out.bushfireProne) {
      if (
        name.includes('bushfire') ||
        results.some(
          (r: any) =>
            JSON.stringify(r).toLowerCase().includes('bushfire prone') ||
            JSON.stringify(r).toLowerCase().includes('bushfire')
        )
      ) {
    ...
    if (!out.foreshoreArea) {
      if (
        name.includes('watercourses') ||
        name.includes('watercourse') ||
        name.includes('foreshore')
      ) {
        out.foreshoreArea = true
      } else if (
        results.some(
          (r: any) =>
            JSON.stringify(r).toLowerCase().includes('watercourse') ||
            JSON.stringify(r).toLowerCase().includes('foreshore')
        )
      ) {
        out.foreshoreArea = true
      }
    ...
    if (
      out.lotSize === null &&
      (name.includes('lot size') ||
        name.includes('lot-size') ||
        name.includes('lotarea') ||
        name.includes('lot size map'))
    ) {
    ...
    if (
      !out.zoneCode &&
      (name.includes('land zoning') || name.includes('zoning') || name.includes('zone'))
    ) {
      const first = results[0] || {}
...
*/
export async function getPlanningAttributes(
  _lat?: number,
  _lon?: number,
  propId?: string
): Promise<PlanningAttributes> {
  ...
  try {
    ...
    try {
      ...
      result.residentZone =
        lotAttrs?.zone || lotAttrs?.ZONING || lotAttrs?.lepZone || lotAttrs?.zoneName || null

      const councilFields = [
        lotAttrs?.lga,
        lotAttrs?.council,
        lotAttrs?.councilName,
        lotAttrs?.localCouncil,
      ]
      const isAlbury = councilFields.some((v: any) =>
        String(v || '')
          .toLowerCase()
          .includes('albury')
      )
