...
const NSW_ADDRESS_API_BASE = 'https://api.apps1.nsw.gov.au/planning/viewersf/V1/ePlanningApi/address';
...
export async function lookupAddress(query: string): Promise<AddressLookupResult | null> {
    ...
    try {
        ...
        if (first) {
            const displayName = first.display || first.formattedAddress || first.fullAddress || first.address || JSON.stringify(first);
            ...
            const propId = first.propId ?? first.propID ?? first.propertyId ?? first.id ?? first.pid ?? first.property_id;
            return {
                displayName,
                street,
                houseNumber,
...
export function isInAlbury(result: AddressLookupResult): boolean {
    ...
    const combined = (result.displayName || '') + ' ' + (result.city || '') + ' ' + JSON.stringify(result.raw || {});
    ...
    const lgaCandidates = [
        raw.lga, raw.localCouncil, raw.councilName, raw.LGA, raw.council
    ];
    if (lgaCandidates.some((v: any) => String(v || '').toLowerCase().includes('albury'))) return true;
    return false;
}