...
export function getRoleSpecificExplanation(role: UserRole, topic: string): string {
  ...
  switch (topic) {
    case 'building_line':
      switch (role) {
        case 'Resident':
          return 'The building line is usually 5m from the street. Your structure must be behind this line.'
        case 'Builder':
          return 'Building line typically 5m from frontage. Check DCP for variations.'
        case 'Purchaser':
          return 'Building line restrictions affect development potential. Verify with council.'
        case 'Planner':
          return 'Building line defined in LEP/DCP. Standard is 5m from frontage.'
        case 'Student':
          return 'Building line is a planning control that keeps buildings away from streets.'
        default:
          return 'Building line requirements vary by location and planning controls.'
      }
    case 'setbacks':
      switch (role) {
        case 'Resident':
          return 'Setbacks keep your structure away from boundaries and neighbors.'
        case 'Builder':
          return 'Minimum setbacks: 0.9m side/rear, 5m front (behind building line).'
        case 'Purchaser':
          return 'Setback requirements limit where structures can be built.'
        case 'Planner':
          return 'Setback requirements vary by zone and structure type per SEPP.'
        case 'Student':
          return 'Setbacks are minimum distances from property boundaries.'
        default:
          return 'Setback requirements depend on zone and structure type.'
      }
    case 'easements':
      switch (role) {
        case 'Resident':
          return 'Easements are areas where utilities have rights. Avoid building there.'
        case 'Builder':
          return 'Check easement locations on survey. Structures cannot be built on easements.'
        case 'Purchaser':
          return 'Easements limit development options. Check property title.'
        case 'Planner':
          return 'Easements are registered interests. SEPP prohibits development on easements.'
        case 'Student':
          return 'Easements are areas where utilities have legal rights to access.'
        default:
          return 'Easements restrict development and must be avoided.'
      }
    case 'heritage':
      switch (role) {
        case 'Resident':
          return 'Heritage areas have special rules. Check with council first.'
        case 'Builder':
          return 'Heritage/conservation overlays require additional approvals.'
        case 'Purchaser':
          return 'Heritage restrictions can significantly limit development.'
        case 'Planner':
          return 'Heritage/conservation overlays override exempt development status.'
        case 'Student':
          return 'Heritage areas have special planning controls to protect character.'
        default:
          return 'Heritage areas require special consideration and council approval.'
      }
    default:
...
export function getRoleSpecificNextSteps(role: UserRole, hasFailures: boolean): string[] {
  ...
  if (!hasFailures) {
    switch (role) {
      case 'Resident':
        return ['You can proceed with construction', 'No additional approvals needed']
      case 'Builder':
        return ['Proceed with construction', 'Document compliance for client']
      case 'Purchaser':
        return ['Development appears feasible', 'Consider professional verification']
      case 'Planner':
        return ['Assessment complete', 'Document for file']
      case 'Student':
        return ['Good example of exempt development', 'Study the requirements']
    }
  }
