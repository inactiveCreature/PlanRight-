...
describe('Switch Accessibility', () => {
  it('toggles_with_keyboard - Space and Enter flip checked state', () => {
    ...
    render(<Switch checked={false} onChange={onChange} ariaLabel="Test switch" />)
  ...
  it('respects_disabled - disabled switch does not toggle via click or keys', () => {
    ...
    render(
      <Switch checked={false} onChange={onChange} disabled={true} ariaLabel="Disabled switch" />
    )
  ...
  it('announces_state - aria-checked updates when toggled', () => {
    ...
    const { rerender } = render(
      <Switch checked={false} onChange={onChange} ariaLabel="Test switch" />
    )
    ...
    rerender(<Switch checked={true} onChange={onChange} ariaLabel="Test switch" />)

    expect(switchElement).toHaveAttribute('aria-checked', 'true')
  })

  it('has_proper_accessibility_attributes', () => {
    render(<Switch checked={false} onChange={vi.fn()} ariaLabel="Test switch" id="test-switch" />)
  ...
  it('supports_different_sizes', () => {
    const { rerender } = render(
      <Switch checked={false} onChange={vi.fn()} size="sm" ariaLabel="Small switch" />
    )

    let switchElement = screen.getByRole('switch')
    expect(switchElement).toHaveClass('w-9', 'h-5')

    rerender(<Switch checked={false} onChange={vi.fn()} size="md" ariaLabel="Medium switch" />)

    switchElement = screen.getByRole('switch')
    expect(switchElement).toHaveClass('w-11', 'h-6')

    rerender(<Switch checked={false} onChange={vi.fn()} size="lg" ariaLabel="Large switch" />)
  ...
  it('applies_custom_className', () => {
    render(
      <Switch checked={false} onChange={vi.fn()} className="custom-class" ariaLabel="Test switch" />
    )
