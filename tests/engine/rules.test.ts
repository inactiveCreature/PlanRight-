...
describe('Rules Engine Verification', () => {
  ...
  it('killer_easement_blocks', () => {
    ...
    expect(result.checks.some((check) => check.rule_id === 'G-SITING-1' && !check.pass)).toBe(true)
  })
  ...
  it('bbl_front_logic', () => {
    ...
    expect(resultBBLTrue.checks.some((check) => check.rule_id === 'S-BBL-1' && check.pass)).toBe(
      true
    )
    ...
    expect(
      resultBBLFalse.checks.some((check) => check.rule_id === 'S-FRONT-1' && !check.pass)
    ).toBe(true)
  })
  ...
  it('decimals_persist', () => {
    ...
    const sideSetbackCheck = result.checks.find(
      (check) => check.rule_id === 'S-SIDE-1' && check.pass
    )
  ...
  it('area_tolerance', () => {
    ...
    const areaCheck = result.checks.find((check) => check.rule_id === 'G-AREA-1')
  ...
  it('heritage_killer_rule', () => {
    ...
    expect(result.decision).toBe('Likely Not Exempt')
    expect(result.checks.some((check) => check.rule_id === 'G-HERITAGE-1' && !check.pass)).toBe(
      true
    )
  })
  ...
  it('shed_attachment_killer', () => {
    ...
    expect(result.decision).toBe('Likely Not Exempt')
    expect(result.checks.some((check) => check.rule_id === 'S-ATTACH-1' && !check.pass)).toBe(true)
  })
